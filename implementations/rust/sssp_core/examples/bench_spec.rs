...
fn run_one(n: usize, avg_degree: f32, seed: u64, check_boundary: bool) -> serde_json::Value {
...
fn main(){
    ...
    if args.iter().any(|a| a=="--help") { eprintln!("Usage: bench_spec --sizes 10000,20000 --degrees 2,4,8 --seed 42 --out benchmarks/native_sample.json [--no-boundary-parity] [--full-parity]"); return; }
    ...
    for s in &sizes { for &deg in &degrees { results.push(run_one(*s, deg, seed, full_parity && !skip_boundary_parity)); } }
